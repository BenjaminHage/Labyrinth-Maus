...
class AutonomousController:
    def __init__(self, angle_pid, wall_distance_pid, point_distance_pid, esc, base_speed, desired_distance, sensor_activation_threshold, wheel_distance, control_distance = 5, angle_toleranz = 0.01, distance_toleranz = 0.03, 
                 esc_angle_comparison_interval = 1, esc_angel_toleranz = 0.8, base_rotation_speed = 0.5, feature_toleranz = 3, direkt_change_toleranz = 15):
...
class Enviroment:
    ...
    def draw_robot(self, x, y, theta, robot_radius):
        """Draw the robot and its orientation on the screen."""
        ...
        end_y = scaled_y + line_length * math.sin(theta)
    ...
    def draw_sensors(self, x, y, theta, sensor_readings, sensor_range, sensor_angles):
        """Draw the sensors and their detection ranges on the screen."""
        scaled_x = int(x * self.zoom_factor)
        scaled_y = int(y * self.zoom_factor)
        ...
        for angle in sensor_angles:
            ...
            if sensor_reading < scaled_sensor_range:
                ...
                end_y = scaled_y + sensor_reading * math.sin(sensor_theta)
            else:
                end_x = scaled_x + scaled_sensor_range * math.cos(sensor_theta)
                end_y = scaled_y + scaled_sensor_range * math.sin(sensor_theta)    
    ...
    def draw_walls(self):
        for wall in self.walls:
            ...
            scaled_rect = pygame.Rect(wall_x * self.zoom_factor, wall_y * self.zoom_factor,
...
def init_Classes():
    ...
    autonomous_controller = AutonomousController(angle_pid, wall_distance_pid, point_distance_pid, esc, init_base_speed, desired_distance, sensor_activation_threshold, 
                                                 robot.get_wheel_distance(), esc_angel_toleranz=esc_angel_toleranz, esc_angle_comparison_interval=esc_angle_comparison_interval, direkt_change_toleranz=direkt_change_toleranz)
